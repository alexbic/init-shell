...
Linger — не отдельный сервис и не дополнительная привилегия: это настройка systemd, которая
запускает менеджер пользовательских служб при загрузке Linux и не останавливает его после
закрытия последней SSH-сессии. Благодаря этому Herdr остаётся доступен через mirror до входа
пользователя и после перезагрузки. Проверка: `loginctl show-user "$USER" -p Linger`; отмена:
