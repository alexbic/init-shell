...
if [[ "$OS_TYPE" == "macos" ]]; then
  ...
  # Удаляем существующую директорию или символическую ссылку .config/wezterm, если она существует
  if [[ -e "$HOME/.config/wezterm" ]]; then
    echo -e "${YELLOW}⚠️ Удаляем существующую конфигурацию WezTerm...${RESET}"
    rm -rf "$HOME/.config/wezterm"
  fi
  
  # Создаем символическую ссылку на конфигурацию WezTerm
  echo -e "${BLUE}🔗 Создаем символическую ссылку для WezTerm...${RESET}"
  create_symlink "$BASE_DIR/wezterm" "$HOME/.config/wezterm"
  
  # Проверяем, что символическая ссылка создана правильно
  if [[ -L "$HOME/.config/wezterm" ]]; then
    target=$(readlink -f "$HOME/.config/wezterm" 2>/dev/null || readlink "$HOME/.config/wezterm")
    if [[ "$target" == "$BASE_DIR/wezterm" ]]; then
      echo -e "${GREEN}✅ Символическая ссылка WezTerm создана правильно${RESET}"
    else
      echo -e "${RED}❌ Символическая ссылка WezTerm указывает на неправильный путь: $target${RESET}"
    fi
  else
    echo -e "${RED}❌ Не удалось создать символическую ссылку для WezTerm${RESET}"
  fi
fi
...
if [[ "$OS_TYPE" == "linux" ]]; then
 ...
 for link in "$HOME/.oh-my-zsh" "$HOME/.vim" "$HOME/.zshrc" "$HOME/.vimrc" "$HOME/.tmux.conf" "$HOME/.tmux.conf.local" "$HOME/.config/wezterm"; do
