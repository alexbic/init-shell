...
# Проверка: запуск только из домашней директории
if [ "$PWD" != "$HOME" ]; then
  echo "❌ Этот скрипт нужно запускать из домашней директории: $HOME"
  exit 1
fi

# Безопасный режим: выходим при ошибке (-e), 
# при использовании необъявленных переменных (-u), 
# и если любая команда в пайпе завершается с ошибкой (pipefail)
set -euo pipefail
...
if [ -d "$BASE_DIR" ]; then
  echo "🗂 Найдена предыдущая установк .myshel.. Делаем резервную копию..."
  mkdir -p "$TMP_BACKUP_DIR"
  rsync -a --exclude "backup" "$BASE_DIR/" "$TMP_BACKUP_DIR/"
  echo "📦 Готовим текущию папку .myshell к бэкапу..."

  # 📁 Создаем каталог backup, только если он не существует или пуст
  ...
  # 📦 Архивируем бэкап из временной папки
...
if [[ -d "$HOME/.oh-my-zsh" ]]; then
  echo "♻️ Обнаружен установленный O- M- Zsh. Выполняем деинсталляцию..."
